...
public partial class Fighter : Task
{
    ...
    public void alpha_init(Vector3 position, Quaternion rotation)
    {
        rigidbody_.init(position, rotation);
        MissileManager.Instance.SetMissileRadius(target_id_, 1f /* radius */);
    ...
    private void alpha_normal_act(float dt, double update_time)
    {
        ...
        if (target_fighter_ != null)
        {
            ...
            if (MyRandom.Probability(3.5f / 6f, dt))
            {
                ...
                for (var i = 0; i < num; ++i)
                {
                    MissileManager.Instance.Spawn(lpos,
    ...
    private void alpha_update(float dt, double update_time)
    {
        ...
        if (update_time - hit_time_ < 0.5f)
        {
            ...
            {
                var torque = MyRandom.PointOnSphere(25f);
